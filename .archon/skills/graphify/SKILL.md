...
```bash
# Detect the correct Python interpreter for graphify
# graphify can be installed via: uv tool, pipx, pip, or system package manager
# Each install method has a different Python path, so we try in order of reliability

PYTHON=""
GRAPHIFY_BIN=$(which graphify 2>/dev/null)

# 1. uv tool installs — most reliable on modern systems
if [ -z "$PYTHON" ] && command -v uv >/dev/null 2>&1; then
    _UV_PY=$(uv tool run --from graphifyy python -c "import sys; print(sys.executable)" 2>/dev/null)
    if [ -n "$_UV_PY" ]; then PYTHON="$_UV_PY"; fi
fi

# 2. pipx installs — binary in ~/.local/bin/, shebang points to managed Python
if [ -z "$PYTHON" ] && [ -n "$GRAPHIFY_BIN" ]; then
    _SHEBANG=$(head -1 "$GRAPHIFY_BIN" 2>/dev/null | tr -d '#! \n')
    case "$_SHEBANG" in
        *[!a-zA-Z0-9/_.-]*) ;;
...
# Write interpreter path so all subsequent steps use the same one
...
### Python interpreter for uv tool installs

On this VPS, graphify is installed via `uv tool install graphifyy`, which creates an isolated virtual environment. The Python binary inside that venv is NOT on PATH, so `which graphify` returns the wrapper script but `import graphify` fails when called via plain `python3`. 

The correct invocation pattern is:
```bash
uv tool run --from graphifyy graphify --help   # CLI
uv tool run --from graphifyy python -c "import graphify; print('OK')"   # verify import
```

The skill's Step 1 handles this automatically via the uv tool detection logic. If a future graphify update changes its package name or uv tool behavior, check here first.

### Hermes + Claude Code integration

This user has both Hermes and Claude Code installed on the VPS. Graphify is installed as a Hermes skill (`/graphify` slash command). For per-project always-on usage, also run:
```bash
graphify install --platform hermes   # already done
graphify install --platform claude    # optional: also install in Claude Code
```

The `graphify claude install` command (CLAUDE.md integration) writes to the local `.claude.md` file in the current directory — only run this in a project directory, not globally.
